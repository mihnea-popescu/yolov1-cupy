...
class Linear:
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        limit = cp.sqrt(1.0 / in_features)
        self.W = cp.random.uniform(-limit, limit, (out_features, in_features))  # shape: (out, in)
        self.b = cp.zeros(out_features) if bias else None  # shape: (out,)

        # Gradients (populated during backward pass)
        self.dW = cp.zeros_like(self.W)
        self.db = cp.zeros_like(self.b) if bias else None
